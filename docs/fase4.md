# Fase 4 — Execução e Análise dos Resultados

## Introdução

Esta fase documenta a **execução das medições de qualidade** do software MEPA, conforme detalhado no **Planejamento da Avaliação (Fase 3)**. O objetivo é verificar e evidenciar a qualidade do sistema frente às características priorizadas: **Adequação Funcional (AF)**, **Eficiência de Desempenho (PE)**, e **Manutenibilidade (MT)**.

Os resultados são apresentados na ordem das prioridades definidas nas Fases 1 e 2.

---

## 1. Adequação Funcional (G-AF-01)

### Objetivo e Método
Avaliar o sistema quanto à **Corretude** das recomendações geradas, comparando os resultados do MEPA com um *oracle* (resultados esperados) via a ferramenta **pytest**.

### AÇÃO NECESSÁRIA (Time de Adequação Funcional)

**Responsáveis:** Davi & Mateus  
**Instruções:** Inserir valores dos testes `pytest`, comparar com os critérios da Fase 3 e preencher a discussão.

### Resultados Obtidos

| Métrica (ID) | Critério de Julgamento | Valor Medido | Julgamento |
|--------------|------------------------|--------------|------------|
| **M-AF-01** – Taxa de Corretude | >= 95% | *[valor]* | *[Conforme / Não Conforme]* |
| **M-AF-02** – Cobertura de Requisitos | >= 80% | *[valor]* | *[Aceitável / Não Aceitável]* |
| **M-AF-03** – Falhas em Casos de Borda | <= 5% | *[valor]* | *[Conforme / Não Conforme]* |
...
## 3. Manutenibilidade (G-MT-01) - Detalhamento Técnico da Execução
